...
from pyne.material_library import (
    MaterialLibrary,
)  # import the material library class from the new location in recent versions of pyne
...
parser.add_argument(
    "-f",
    dest="filenamein",
    help="Name of the pyne material database file to read...specify full directory path e.g. -f /home/smith/.local/lib/python2.7/site-packages/pyne/nuc_data.h5 or -f testelliot/testlibsh6_70c.h5m",
)
parser.add_argument(
    "-m",
    "--writeMCNP",
    help="Write all materials in MCNP matl card format to 2 text files (atom and mass fraction)",
    action="store_true",
)
parser.add_argument(
    "-c",
...
parser.add_argument(
    "-l",
    "--writeAlara",
    help="Write all materials in Alara matl card format to a text file",
    action="store_true",
)
parser.add_argument(
    "-j",
    "--writeJson",
    help="Write all materials in Json matl card format to individual files",
    action="store_true",
)
parser.add_argument(
    "-p",
    "--prebuilt",
    help="Assume prebuilt matl library distributed with PyNE that has non-default datapath,nucpath",
    action="store_true",
)
parser.add_argument(
    "-w",
...
#
pynematdatabasefilein = (
    args.filenamein
    if args.filenamein is not None
    else "/home/smith/.local/lib/python2.7/site-packages/pyne/nuc_data.h5"
)
#
#
...
for matkey, matvalue in matllib.items():
    print(
        ...
        matkey.decode("utf8"),
        "The mass density is: ",
        matllib[matkey].mass_density(),
        "atom density",
        "{:11.4e}".format(matllib[matkey].number_density() / 1.0e24),
    )
#
...
print(
    "            molecular mass: ",
    matllib[input_string].molecular_mass(),
    " mass density: ",
    matllib[input_string].mass_density(),
)
print(
    "\n An mcnp material print of input_string using mass fractions...\n",
    matllib[input_string].mcnp(),
)  # mass fraction is the default
print(
    "\n An mcnp material print of input_string using atom fractions...\n",
    matllib[input_string].mcnp("atom"),
)  # use atom fraction
print(
...
if args.writeMCNP:
    ...
    for matkey, matvalue in matllib.items():
        print(
            "   Writing ",
            matkey.decode("utf8"),
            "to a file (testplayallmat_mcnpxxx.txt) using MCNP format and atom/mass fractions...  \n",
        )
...
if args.writeAlara:
    ...
    for matkey, matvalue in matllib.items():
        print(
            "   Writing ",
            matkey.decode("utf8"),
            "to a file (testplayallmat_alara.txt) using Alara format ...  \n",
        )
        testmat = matllib[matkey]
...
if args.writeJson:
    ...
    for matkey, matvalue in matllib.items():
        ...
        testmat.write_json(
            jsonFileName
        )  # doesn't append so only last material in list appears
#
